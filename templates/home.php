<div class="container-fluid map-container">
    <div class="row">
        <div class="col-xs-6" id="globeParent">
            <h1>test</h1>
...
<script>
    ...
    function ready(error, world, names) {
        ...
        var globe = {type: "Sphere"},
            ...
            i = -1;
        console.log(globe, land, countries);
        ...
        var zoomBehavior = d3.behavior.zoom().scaleExtent([1, 10]).on("zoom", function () {
            console.log('zoom');
            // console.log(d3.event.translate);
            // console.log(d3.event.scale);
            // canvas.attr("transform", "translate(" + d3.event.translate + ")");
            // canvas.attr("style", "transform:scale(" + d3.event.scale + ")");
            // d3.geo.path()
            // .projection(d3.geo.orthographic() .scale(500) .clipAngle(90) .translate([width/2,height/2]))
            // .context(c);
            console.log(d3.event.scale);
            console.log("projection.scale() : "+projection.scale());
            projection.scale(500*d3.event.scale);
            redraw()
            // var rotation = projection.rotate();
            // var radius = projection.scale();
            // var scale = d3.scale.linear()
        ...
        function customTransition(journey){
            ...
            d3.transition()
                ...
                .tween("rotate", function() {
                    ...
                    redraw();
                    d3.select("#globeParent").select('canvas').call(dragBehaviour).call(zoomBehavior);
                })
        }
    ...
    queue()
        .defer(d3.json, "<?php echo BASE_URL; ?>assets/json/world.json")
        .defer(d3.tsv, "<?php echo BASE_URL; ?>assets/tsv/world-country-names.tsv")
        .await(ready);
</script>
